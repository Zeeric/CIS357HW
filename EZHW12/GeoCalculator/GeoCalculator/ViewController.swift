...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var originTemp: GeoCalcLabel!
    @IBOutlet weak var originDesc: GeoCalcLabel!
    @IBOutlet weak var destIcon: UIImageView!
    @IBOutlet weak var destTemp: GeoCalcLabel!
    @IBOutlet weak var destDesc: GeoCalcLabel!
    ...
    func clearWeatherViews() {
        self.originDesc.text = ""
        self.originTemp.text = ""
        self.originIcon.image = nil
        self.destDesc.text = ""
        self.destTemp.text = ""
    ...
    func doCalculatations()
    {
        ...
        wAPI.getWeatherForDate(date: Date(), forLocation: (p1lt, p1ln)) { (weather) in
            if let w = weather {
                DispatchQueue.main.async {
                    self.originTemp.text = "\(w.temperature.roundTo(places: 1))"
                    self.originIcon.image = UIImage(named: w.iconName)
                    self.originDesc.text = w.summary
                }
            }
        }
        ...
        wAPI.getWeatherForDate(date: Date(), forLocation: (p2lt, p2ln)) { (weather) in
            if let w = weather {
                DispatchQueue.main.async {
                    self.destTemp.text = "\(w.temperature.roundTo(places: 1))"
                    self.destIcon.image = UIImage(named: w.iconName)
                    self.destDesc.text = w.summary
                }
            }
        }
